...
function ProjectsTable() {
    ...
    const handleDelete = async (id:string) => {
        try {
            await dbAppwriteService.deleteData(id)
            dispatch(deletePoject(id))
        } catch (error) {
            console.log(error);
        }
    }
            ...
            <div className="max-h-[28em] overflow-y-auto">
                <table className="w-full text-left">
                    ...
                    {projectData && projectData.length > 0 ? (
                        <tbody className="divide-y divide-stone-200 dark:divide-(--zd125-color) mask-b-from-90%">
                                ...
                                <tr key={prop.$id} className="group dark:bg-[#18181b] hover:bg-gray-50 dark:hover:bg-gray-800/50 transition">
                                    ...
                                    <td className="flex justify-between items-center px-6 py-4 text-sm text-stone-600 dark:text-(--zd5-color)">
                                        ...
                                        <span onClick={()=> handleDelete(prop.$id)} className="cursor-pointer ">
                                            <MdDelete className="opacity-0 translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-200 text-red-500 text-xl"/>
                                        </span> 
                                    </td>
                                </tr>
                            ))}
